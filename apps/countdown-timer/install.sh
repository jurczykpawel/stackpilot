...
if build_cf_snippet; then
    ...
    echo "  WARN: failed to fetch Cloudflare IP ranges (snippet not written)"
fi

# Weekly refresh — CF ranges rotate occasionally
cat > /etc/cron.weekly/cf-ip-refresh << 'WCRON'
#!/bin/sh
v4=$(curl -fsSL --max-time 10 https://www.cloudflare.com/ips-v4 | tr '\n' ' ')
v6=$(curl -fsSL --max-time 10 https://www.cloudflare.com/ips-v6 | tr '\n' ' ')
[ -z "$v4" ] || [ -z "$v6" ] && exit 0
